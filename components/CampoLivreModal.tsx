...
// ── Main component ─────────────────────────────────────────────────────────
// ── Neon DB direct HTTP save ─────────────────────────────────────────────────
async function saveConversationNeon(connStr: string, sessionId: string, messages: CLMessage[], mode: string): Promise<void> {
  try {
    const host = connStr.match(/@([^/]+)\//)?.[1] ?? "";
    const db = connStr.match(/\/([^?]+)(\?|$)/)?.[1] ?? "neondb";
    const user = connStr.match(/\/\/([^:]+):/)?.[1] ?? "";
    const pass = connStr.match(/:([^@]+)@/)?.[1] ?? "";
    const projectId = connStr.match(/([a-z0-9-]+)\.neon\.tech/)?.[1] ?? "";
    if (!projectId) return;
    const neonApiUrl = `https://${projectId}.neon.tech/sql`;
    const auth = "Basic " + btoa(`${user}:${pass}`);
    const lastUser = messages.filter(m => m.role === "user").slice(-1)[0]?.content ?? "";
    const lastAI = messages.filter(m => m.role === "assistant").slice(-1)[0]?.content ?? "";
    await fetch(neonApiUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": auth, "Neon-Connection-String": connStr },
      body: JSON.stringify({
        query: `INSERT INTO dm_mensagens (id, conversa_id, role, content, created_at) VALUES ($1,$2,$3,$4,NOW()) ON CONFLICT DO NOTHING`,
        params: [Date.now().toString() + "_u", sessionId, "user", lastUser.slice(0, 2000)],
      }),
    ...
    await fetch(neonApiUrl, {
      ...
      body: JSON.stringify({
        query: `INSERT INTO dm_mensagens (id, conversa_id, role, content, created_at) VALUES ($1,$2,$3,$4,NOW()) ON CONFLICT DO NOTHING`,
        params: [Date.now().toString() + "_a", sessionId, "assistant", lastAI.slice(0, 4000)],
      }),
      signal: AbortSignal.timeout(5000),
    });
    await fetch(neonApiUrl, {
      ...
      headers: { "Content-Type": "application/json", "Authorization": auth, "Neon-Connection-String": connStr },
      body: JSON.stringify({
        query: `INSERT INTO dm_conversas (id, titulo, modo, total_msgs, updated_at) VALUES ($1,$2,$3,$4,NOW()) ON CONFLICT (id) DO UPDATE SET total_msgs=$4, updated_at=NOW()`,
        params: [sessionId, lastUser.slice(0, 60) || "Campo Livre", mode, messages.length],
      }),
      signal: AbortSignal.timeout(5000),
    });
...
export default function CampoLivreModal({ visible, onClose }: Props) {
  ...
  const { settings, updateSettings, dbConfigs } = useApp();
  ...
  const sessionId = useRef<string>(`cl_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`);
  ...
  const sendMessage = useCallback(async (text?: string) => {
    ...
    try {
      ...
      const activeLegalMode = settings.legalMode ?? "advogado";
      const legalModeData = getLegalMode(activeLegalMode);
      const baseSysPt = isPplx ? PPLX_SYSTEM_PROMPT : legalModeData.prompt;
      ...
      if (!accumulated) {
        ...
        if (autoSpeak && !abortRef.current) {
          setSpeaking(aiMsg.id);
          await speakText(accumulated, ttsSettings);
          setSpeaking(null);
        }
        // Salva no Neon DB se houver configuração
        const neonCfg = dbConfigs?.find(c => c.provider === "neon" || c.connectionString?.includes("neon.tech"));
        const neonConn = neonCfg?.connectionString || "";
        if (neonConn) {
          const finalMsgs = [...history, { id: aiMsg.id, role: "assistant" as const, content: accumulated }];
          saveConversationNeon(neonConn, sessionId.current, finalMsgs, settings.legalMode ?? "livre");
        }
      }
    } catch (e) {
      if (!abortRef.current) {
  ...
  }, [messages, input, loading, apiKey, detected, isPplx, webSearch, doWebSearch, autoSpeak, ttsSettings, settings, dbConfigs]);
  ...
  return (
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={[clStyles.container, { backgroundColor: colors.background }]}>
        ...
        {/* ── Seletor de modos jurídicos globais ── */}
        <View style={{ flexDirection: "row", paddingHorizontal: 10, paddingVertical: 6, gap: 5, backgroundColor: colors.secondary, borderBottomWidth: 1, borderBottomColor: colors.border }}>
          {LEGAL_MODES.map((lm) => {
            const active = (settings.legalMode ?? "advogado") === lm.key;
            return (
              <TouchableOpacity
                key={lm.key}
                onPress={() => { updateSettings({ legalMode: lm.key }); setMessages([]); sessionId.current = `cl_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`; Haptics.selectionAsync(); }}
                style={{
              ...
              >
                ...
                <Text style={{ fontSize: 9, fontWeight: active ? "700" : "400", color: active ? lm.color : colors.mutedForeground, marginTop: 1, textAlign: "center" }}>
                  {lm.label}
                </Text>
                {lm.hasInternet && <Text style={{ fontSize: 8, color: lm.color, opacity: 0.8 }}>🌐</Text>}
              </TouchableOpacity>
            );
          })}
        </View>
